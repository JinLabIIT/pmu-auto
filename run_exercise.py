		...
		for link in host_links:
			...
			if host_name == "h1":
				self.addLink(host_name, sw_name,
                                                 delay=link['latency'], bw=link['bandwidth'], loss=LOSS, port2=sw_port)

			else:
				self.addLink(host_name, sw_name,
                                                 delay=link['latency'], bw=link['bandwidth'], port2=sw_port)
...
class ExerciseRunner:
	...
	def program_switch_cli(self, sw_name, sw_dict):
		...
		sub_env["PYTHONPATH"] = pythonpath + \
								"/home/gong/Workspace/P4/behavioral-model/thrift_src/gen-py/"
		...
		with open(cli_input_commands, 'r') as fin:
			...
			with open(cli_outfile, 'w') as fout:
				subprocess.Popen([cli, '--thrift-port', str(thrift_port)],
								 stdin=fin, stdout=fout, env = sub_env)
